...
complex(8) :: propZ,propPh,BosonicPartAmp(1:3,-2:1),mydummy,ZPolVec(1:4),BarSpi(1:4),Spi(1:4)
...
real(8) :: eta1,eta2,sHatJacobi,MZJacobi,PreFac,FluxFac,PDFFac,AccPoles
...
!      print *, 'QPtol:', QPtol
! 
!      print *, 'top-Z coupling',couplZTT_left_dyn,couplZTT_right_dyn
!      first=.false.
!   endif
...
ENDIF
IF( TTBZ_DebugSwitch.EQ.0 .OR. TTBZ_DebugSwitch.EQ.3 ) THEN
   NLO_Res_UnPol( 0) = NLO_Res_UnPol(0)  - Ren_Res_UnPol*2d0   ! Raoul's gamma5 ren -- CF now included in subroutine
ENDIF
...
! LO_Res_Unpol       = LO_Res_Unpol       * PreFac
   ...
   IF( TOPDECAYS.GE.1 ) THEN
       if( .not. HelSampling .and. ZDecays.ge.10 ) xE = yRnd(19)
       if( HelSampling       .and. ZDecays.ge.10 ) xE = yRnd(19)
       if( .not. HelSampling .and. ZDecays.lt.10 ) xE = yRnd(18)
       if( HelSampling       .and. ZDecays.lt.10 ) xE = yRnd(18)
   ELSEIF( TOPDECAYS.EQ.0 ) THEN
       if( .not. HelSampling .and. ZDecays.ge.10 ) xE = yRnd(9)
       if( HelSampling       .and. ZDecays.ge.10 ) xE = yRnd(9)       
       if( .not. HelSampling .and. ZDecays.lt.10 ) xE = yRnd(8)
       if( HelSampling       .and. ZDecays.lt.10 ) xE = yRnd(8)
   ENDIF
...
!    stop
   ...
   if( IsNan(EvalCS_1L_ttbggZ) ) then
        ...
        print *, PSWgt , VgsWgt , PDFFac, sHatJacobi, mzJacobi
...
complex(8) :: propZ,propPh,ZPolVec(1:4),BarSpi(1:4),Spi(1:4),light_quark_coupl(1:2)
logical :: applyPSCut,nonrenormcoupl
logical, save :: first=.true.
...
real(8) :: tau,eta1,eta2,sHatJacobi,MZJacobi,PreFac,FluxFac,PDFFac_a(1:2),PDFFac_b(1:2),PDFFac(1:2),pdf(-6:6,1:2)
...
! this should not be changed...
  ...
  if (cdabs(couplZTT_V2) .ge. 1d-7 .or. cdabs(couplZTT_A2) .ge. 1d-7) then
     ...
     ! if V2 and A2 are zero, then poles work
...
!     do iHel=1,NumHelicities
       ...
       elseif( Zdecays.gt.10 ) then
         LOPartAmp(up)=BornAmps(1)%Result + BornAmps(2)%Result*( couplZUU*propZ*couplZLL + couplGUU*propPh*couplGLL )
         LOPartAmp(dn)=BornAmps(1)%Result + BornAmps(2)%Result*( couplZDD*propZ*couplZLL + couplGDD*propPh*couplGLL )
       endif
...
!!!! RR Higher Order Operator -- i.e. sigma_{mu,nu} q^{nu} --  renorm term ??????
      ...
      elseif( Zdecays.gt.10 ) then
         light_quark_coupl(up)= couplZUU*propZ*couplZLL + couplGUU*propPh*couplGLL 
         light_quark_coupl(dn)= couplZDD*propZ*couplZLL + couplGDD*propPh*couplGLL 
      elseif (Zdecays .eq. -2)  then    ! this is the photon comparison
...
IF( TTBZ_DebugSwitch.EQ.0 .OR. TTBZ_DebugSwitch.EQ.3 ) THEN
   ...
   Ren_Res_Pol = 4d0/3d0*ColLO_ttbqqb(1,1) * dreal( LOPartAmp(up)*dconjg(RenormAmp(up))*PDFFac(up) + LOPartAmp(dn)*dconjg(RenormAmp(dn))*PDFFac(dn) )     ! 4/3=CF is vertex color factor
   Ren_Res_UnPol=Ren_Res_UnPol + Ren_Res_Pol
ENDIF
...
IF( TTBZ_DebugSwitch.EQ.0 .OR. TTBZ_DebugSwitch.EQ.3 ) THEN
   NLO_Res_UnPol( 0)=NLO_Res_UnPol(0) - Ren_Res_UnPol*2d0  ! Raoul's gamma5 ren
ENDIF
...
! print *, "LO",LO_Res_Unpol/(100d0**2) , LO_Res_Unpol
! print *, "1L check",(NLO_Res_UnPol(-2)+ NLO_Res_UnPol_Ferm(-2) )                          /(alpha_sOver2Pi*RunFactor)/(LO_Res_Unpol)
! print *, "1L check",(NLO_Res_UnPol(-1)+ NLO_Res_UnPol_Ferm(-1) )                          /(alpha_sOver2Pi*RunFactor)/(LO_Res_Unpol)
! print *, "1L check",(NLO_Res_UnPol(0) + NLO_Res_UnPol(1) + NLO_Res_UnPol_Ferm(0) + NLO_Res_UnPol_Ferm(1) )   /(alpha_sOver2Pi*RunFactor)/(LO_Res_Unpol)
! pause
...
real(8) :: eta1,eta2,MZJacobi,sHatJacobi,FluxFac,PDFFac,MZ_Inv
...
IF( ZDECAYS.NE.0 ) THEN
   call EvalPhasespace_ZDecay(MomExt(1:4,4),yRnd(19:20),MomExt(1:4,13:14),PSWgt4)
   PSWgt = PSWgt * PSWgt4 * MZJacobi
ENDIF
...
complex(8) :: LO_Res_Pol,LO_Res_Unpol,PropZ,propPh,LOPartAmp(1:4,1:2)
...
real(8) :: eta1,eta2,MZJacobi,sHatJacobi,FluxFac,PDFFac(1:2),PDFFac_a(1:2),PDFFac_b(1:2),PObs(1:NumMaxHisto)
...
ENDIF
   ...
   if ( ZDecays .lt. 10) then
      propZ = (1d0,0d0)/dsqrt(2d0*Ga_Zexp*m_Z)
   elseif (ZDecays .gt. 10) then 
      propZ=(1d0,0d0)/(MZ_Inv**2-m_Z**2+ci*Ga_ZExp*m_Z)
   endif
...
!         if( TTBZ_SpeedUp ) nHel(2)=NumHelicities/2
        do iHel=nHel(1),nHel(2)
          ...
          elseif( Zdecays.gt.10 ) then
            LOPartAmp(1,Up_:Dn_) = BornAmps(PrimAmp1_162345)%Result * (/1d0,1d0/) &       !for tops propZ*couplZLL is multiplied inside mod_ZDecay
                                 + BornAmps(PrimAmp1_123645)%Result * ( (/couplZUU,couplZDD/)*propZ*couplZLL + (/couplGUU,couplGDD/)*propPh*couplGLL )
            LOPartAmp(2,Up_:Dn_) = BornAmps(PrimAmp1_165234)%Result * (/1d0,1d0/)  &
                                 + BornAmps(PrimAmp1_152364)%Result * ( (/couplZUU,couplZDD/)*propZ*couplZLL + (/couplGUU,couplGDD/)*propPh*couplGLL )
            LOPartAmp(3,Up_:Dn_) = BornAmps(PrimAmp1_162534)%Result * (/1d0,1d0/)  &
                                 + BornAmps(PrimAmp1_125364)%Result * ( (/couplZUU,couplZDD/)*propZ*couplZLL + (/couplGUU,couplGDD/)*propPh*couplGLL )
            LOPartAmp(4,Up_:Dn_) = BornAmps(PrimAmp1_162354)%Result * (/1d0,1d0/)  &
                                 + BornAmps(PrimAmp1_123654)%Result * ( (/couplZUU,couplZDD/)*propZ*couplZLL + (/couplGUU,couplGDD/)*propPh*couplGLL )
          endif
...
!needed for dipole check  call swapMom(MomExt(1:4,1),MomExt(1:4,2))
...
complex(8) :: LO_Res_Pol,LO_Res_Unpol,Dip_Res_Unpol,NLO_Res_Pol,NLO_Res_UnPol,propZ,propPh
...
logical :: applyPSCut,applySingCut
real(8) :: tau,eta1,eta2,MZJacobi,sHatJacobi,PreFac,FluxFac,PDFFac_a(1:2),PDFFac_b(1:2),PDFFac(1:2),RunFactor
real(8) :: pdf(-6:6,1:2)
real(8) :: couplZUU,couplZDD,couplZLL,couplGUU,couplGDD,couplGLL,couplZTT,couplGTT
...
!----------------------------------------
   ...
   PreFac = fbGeV2 * FluxFac * sHatJacobi * PSWgt1*PSWgt2*PSWgt3*PSWgt4 * MZJacobi * VgsWgt
...
do npdf=1,2
   ...
   do iHel=1,NumHelicities
      ...
      elseif(PROCESS.EQ.72) then
         ...
         if( Zdecays.ge.10 ) then
             LOPartAmp(up)  = LOPartAmp(up)  + couplGUU*propPh*couplGLL*TreeResult(2)
             LOPartAmp(dn)  = LOPartAmp(dn)  + couplGDD*propPh*couplGLL*TreeResult(2)
             NLOPartAmp(up) = NLOPartAmp(up) + couplGUU*propPh*couplGLL*DKResult(2)
             NLOPartAmp(dn) = NLOPartAmp(dn) + couplGDD*propPh*couplGLL*DKResult(2)
         endif
...
!----------------------------------------
   ...
   call EvalPhasespace_ZDecay(MomExt(1:4,3),yRnd(19:20),MomExt(1:4,13:14),PSWgt4)
...
do npdf=1,2
   ...
   do GluHel=1,-1,-2 ! loop over additional gluon chiralities from decay
      ...
      elseif(PROCESS.EQ.72) then
         ...
         if( Zdecays.ge.10 ) then
             LOPartAmp(up)  = LOPartAmp(up)  + couplGUU*propPh*couplGLL*BornAmps(2)%Result
             LOPartAmp(dn)  = LOPartAmp(dn)  + couplGDD*propPh*couplGLL*BornAmps(2)%Result
         endif
         LO_Res_Pol    = ColLO_ttbqqb(1,1) * ( LOPartAmp(up)*dconjg(LOPartAmp(up))*PDFFac(up) + LOPartAmp(dn)*dconjg(LOPartAmp(dn))*PDFFac(dn))
      endif
      LO_Res_UnPol = LO_Res_UnPol + LO_Res_Pol
...
! now set the SM-like couplings to zero, and recalculate the LO amps
    ...
    if( ZDecays.gt.0 ) then
       call ZDecay(ExtParticle(5),DK_LO,plept(1:4,1:2))
    else
       couplZTT_left_dyn=couplZTT_left
       couplZTT_right_dyn=couplZTT_right
    endif
...
! now set all vector couplings to zero (i.e. L=-R=A) and recompute Born ampls
    couplZTT_left  = couplZTT_A
    couplZTT_right =-couplZTT_A
    couplZTT_left2 = couplZTT_A2
    couplZTT_right2 =-couplZTT_A2
    Q_Top=0d0
...
! don't include amplitudes with Z attached to qqb line
  ...
  end subroutine STORETOPZCOUPLINGS
  


  subroutine RetrieveTopZCouplings
    use ModParameters
    implicit none
  ...
  end subroutine RetrieveTopZCouplings
