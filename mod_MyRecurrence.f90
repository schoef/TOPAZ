...
END FUNCTION
      ...
      if( abs(QuarkFlavor).eq.Top_ .or. abs(QuarkFlavor).eq.Bot_ ) then!   note that Bot_ is treated as top quark in TOPAZ!
            ...
            couplVQQ_left2 = couplZTT_left2_dyn
            couplVQQ_right2 = couplZTT_right2_dyn
      else
         couplVQQ_left=couplZQQ_left_dyn
         couplVQQ_right=couplZQQ_right_dyn
! q-couplings are always zero for light quarks
         couplVQQ_left2=czero
         couplVQQ_right2=czero
      endif



if (ngluon == 0) then
         res = vbqV(sp,eV,couplVQQ_left,couplVQQ_right,couplVQQ_left2,couplVQQ_right2,kV,ColorlessTag)
else
       ...
       do m=0,ng2-1
        ...
        tmp = vbqV(sp2,eV,couplVQQ_left,couplVQQ_right,couplVQQ_left2,couplVQQ_right2,kV,ColorlessTag)
        if (abs(k2sq) > propcut) then
               tmp = (0d0,1d0)/k2sq*tmp
        else
                tmp = (0d0,0d0)
...
if (ngluon == 0) then
         res = vVq(eV,sp,couplVQQ_left,couplVQQ_right,couplVQQ_left2,couplVQQ_right2,kV,ColorlessTag)
else
...
!        sp2 = spb2_(sp2,k2) +mass*sp2
        ...
        tmp = vVq(eV,sp2,couplVQQ_left,couplVQQ_right,couplVQQ_left2,couplVQQ_right2,kV,ColorlessTag)
        if (abs(k2sq) > propcut) then
               tmp = (0d0,1d0)/k2sq*tmp
        else 
               tmp = (0d0,0d0)
