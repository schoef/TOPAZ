...
FUNCTION PoissonDistr(Nobs,Nexp)
implicit none
real(8) :: PoissonDistr
integer :: Nobs,Nexp
integer :: NobsFactorial,i
   

  NobsFactorial=1
  do i=1,Nobs
    NobsFactorial=NobsFactorial*i
  enddo

print *, "xxx",(Nexp**Nobs) , (dble(-Nexp)) , dble(NobsFactorial)

  PoissonDistr = (Nexp**Nobs) * dexp(dble(-Nexp)) / dble(NobsFactorial)
...
FUNCTION GaussDistr(x,mu)
implicit none
real(8) :: GaussDistr,sigma,x,mu
   

  sigma = dsqrt(x)
  GaussDistr = 1d0/sigma/dsqrt(2d0*3.14159265d0) * dexp(-(x-mu)**2/2d0/sigma**2)
...
!--------------------------------------------------
  ...
  enddo
  NumExpectedEvents(1) = int(Lumi * sigmatot(1) *8 )
...
!   NumEvents = NumExpectedEvents(1)
  NumEvents = NumExpectedEvents(iHypothesis)
...
!************************************************************
      ! find max. of y-axis
      ymax=-1d13;
      do iBin=NumEvents-50,NumEvents+50
         ymax =  GaussDistr(dble(ibin),dble(NumEvents))
         print *, ibin, ymax
      enddo
      pause
...
!          LLRatio = LLRatio + 2d0*dlog( (sigmatot(1)*Value(1,SelectedEvent))/(sigmatot(2)*Value(2,SelectedEvent)) )
!          LLRatio = LLRatio + 2d0*dlog( (Value(1,SelectedEvent))/(Value(2,SelectedEvent)) )
      enddo
      LLRatio = LLRatio + NumEvents*dlog(dble(NumExpectedEvents(1))/dble(NumExpectedEvents(2))) - (NumExpectedEvents(1)-NumExpectedEvents(2))
!       LLRatio = LLRatio - 2d0*Lumi*(sigmatot(1)-sigmatot(2))
!       LLRatio = LLRatio + NumEvents*dlog(dble(NumExpectedEvents(1))/dble(NumExpectedEvents(2))) - (NumExpectedEvents(1)-NumExpectedEvents(2))
