...
ELSEIF( ObsSet.EQ.82 ) THEN! set of observables for ttb+H (di-leptonic tops)
         ...
         Histo(3)%Info   = "y(top)"
         Histo(3)%NBins  = 16
         Histo(3)%BinSize= 0.50d0
         Histo(3)%LowVal =  -4d0
         Histo(3)%SetScale= 1d0

         Histo(4)%Info   = "y(Higgs)"
         Histo(4)%NBins  = 16
         Histo(4)%BinSize= 0.50d0
         Histo(4)%LowVal =  -4d0
         Histo(4)%SetScale= 1d0

         Histo(5)%Info   = "m(t+tb+H)"
         Histo(5)%NBins  = 20
         Histo(5)%BinSize= 20d0*GeV
         Histo(5)%LowVal =  460d0*GeV
         Histo(5)%SetScale= 100d0

!! 2-d histogram
!! NB -- 2-dim histoggrams MUST have "2D_" as the first three characters of the info
!! -- this is to ensure that they are properly written out
         Histo2D(1)%Info  = "2D_pT(top),pT(Higgs)"
         Histo2D(1)%HistoDim=2
         Histo2D(1)%NBins2=Histo(1)%NBins
         Histo2D(1)%NBins3=Histo(2)%NBins
         Histo2D(1)%NBins=(Histo2D(1)%NBins2)*(Histo2D(1)%NBins3)
         Histo2D(1)%BinSize= 1d0
         Histo2D(1)%LowVal =  0d0*GeV
         Histo2D(1)%SetScale =  100d0
         
         Histo2D(1)%BinSize2= Histo(1)%BinSize
         Histo2D(1)%LowVal2 =  Histo(1)%LowVal
         Histo2D(1)%SetScale2= Histo(1)%SetScale

         Histo2D(1)%BinSize3= Histo(2)%BinSize
         Histo2D(1)%LowVal3 =  Histo(2)%LowVal
         Histo2D(1)%SetScale3= Histo(2)%SetScale



         Histo2D(2)%Info  = "2D_pT(top),y(Higgs)"
         Histo2D(2)%HistoDim=2
         Histo2D(2)%NBins2=Histo(1)%NBins
         Histo2D(2)%NBins3=Histo(4)%NBins
         Histo2D(2)%NBins=(Histo2D(2)%NBins2)*(Histo2D(2)%NBins3)
         Histo2D(2)%BinSize= 1d0
         Histo2D(2)%LowVal =  0d0
         Histo2D(2)%SetScale =  1d0
         
         Histo2D(2)%BinSize2= Histo(1)%BinSize
         Histo2D(2)%LowVal2 =  Histo(1)%LowVal
         Histo2D(2)%SetScale2= Histo(1)%SetScale
         Histo2D(2)%BinSize3= Histo(4)%BinSize
         Histo2D(2)%LowVal3 =  Histo(4)%LowVal
         Histo2D(2)%SetScale3= Histo(4)%SetScale





         Histo2D(3)%Info  = "2D_y(top),m_ttbH"
         Histo2D(3)%HistoDim=2
         Histo2D(3)%NBins2=Histo(3)%NBins
         Histo2D(3)%NBins3=Histo(5)%NBins
         Histo2D(3)%NBins=(Histo2D(3)%NBins2)*(Histo2D(3)%NBins3)
         Histo2D(3)%BinSize= 1d0
         Histo2D(3)%LowVal =  0d0
         Histo2D(3)%SetScale =  1d0
         
         Histo2D(3)%BinSize2= Histo(3)%BinSize
         Histo2D(3)%LowVal2 =  Histo(3)%LowVal
         Histo2D(3)%SetScale2= Histo(3)%SetScale
         Histo2D(3)%BinSize3= Histo(5)%BinSize
         Histo2D(3)%LowVal3 =  Histo(5)%LowVal
         Histo2D(3)%SetScale3= Histo(5)%SetScale
...
real(8),optional :: PObs(:)
real(8) :: nZLept,s12,s13,s14,s23,s24,s34
real(8) :: pT_lep(4),ET_miss,PT_miss,pT_ATop,pT_Top,pT_Higgs,HT,ET_bjet
real(8) :: y_ATop,y_Top,y_higgs,y_lep(1:4),pseudo_Z, pseudo_top, pseudo_tbar
real(8) :: pT_jet(1:7),eta_jet(1:7),eta_sepa,mT_bln(1:2),pT_Z,eta_Z,m_ttbH
real(8) :: R_lj(1:5),R_PlepM,pT_lept,ET_lept,mT,dPhiLL,CosTheta1,DphiZt,Dphittbar
...
elseif( ObsSet.eq.82) then! ttb+H production with di-leptonic tops
    ...
    m_ttbH = get_MInv(Mom(1:4,tbar)+Mom(1:4,t)+Mom(1:4,Hig))

!   binning
    NBin(1) = WhichBin(1,pT_Top)
    NBin(2) = WhichBin(2,pT_Higgs)
    NBin(3) = WhichBin(3,y_Top)
    NBin(4) = WhichBin(4,y_Higgs)
    NBin(5) = WhichBin(5,m_ttbH)
! for the multidimensional histogram, use the above bins
    NBin(6) = Convert2DHist(NBin(1),NBin(2),Histo(1)%NBins,Histo(2)%NBins) 
!!NBin(5) = Convert2DHist(NBin(1),NBin(1),Histo(1)%NBins,Histo(2)%NBins) 
    NBin(7) = Convert2DHist(NBin(1),NBin(4),Histo(1)%NBins,Histo(4)%NBins) 
    NBin(8) = Convert2DHist(NBin(3),NBin(5),Histo(3)%NBins,Histo(5)%NBins) 
!    if( present(PObs) ) then
!      PObs(1) = pT_Top
!      PObs(2) = pT_Higgs
!      PObs(3) = y_Top
...
FUNCTION Convert2DHist(nx,ny,totx,toty)
  ...
  if (nx .gt. totx .or. ny .gt. toty) then
