module ModIntDipoles_QBGTTBQBH
use ModParameters
use ModProcess
use ModMisc
implicit none
...
public:: EvalIntDipoles_QBGTTBQBH
...
!     outgoing convention:
!     bar t = 1, H =2, t=3 ,  incoming quark=4,
!     5=incoming gluon
!-----------------------------------------------------------

      subroutine EvalIntDipoles_QBGTTBQBH(p,MomDK,z,npdf,res)
      use ModIntDipoles
      use ModAmplitudes
      implicit none
      ...
      type(TreeProcess),save :: TreeAmpsDip2(1:1)
      complex(dp) :: B(1:2),Bm(1:2,1:2),mtrsq(2),MZ_Inv,propZ,propP
      ...
      if( npdf.eq.2 ) then
       ...
       if( first_time1 ) then
             ...
             call InitProcess_TbTGGH(ExtParticles1(1:5))
...
! spin & color avg., color convention adjustment
      mtrsq(1) = mtrsq(1) * alpha_s4Pi**2   /4d0/64d0
...
!-- ---- NEXT DIPOLE
      AM = (zero,zero) ! first, the ``amplitude matrix'' is set to zero
       ...
       if( first_time2 ) then
             call InitTrees(4,0,1,TreeAmpsDip2,NumBoson=1)
             call InitProcess_TbTQbQH(ExtParticles2(1:5))
...
!                 MZ_Inv = dsqrt(dabs((p(1:4,2)).dot.(p(1:4,2))))
!           endif
...
!                   Bm(dn_,i6)=B(i6)
!                   if( HDecays.gt.0 .and. HDecays.lt.10 ) then
!                       Bm(up_,i6)=Bm(up_,i6)*propZ*couplZLL*couplZUU
!                       Bm(dn_,i6)=Bm(dn_,i6)*propZ*couplZLL*couplZDD
!                   elseif( HDecays.gt.10 ) then
!                       Bm(up_,i6)=Bm(up_,i6) * ( propZ*couplZLL*couplZUU + propP*couplGLL*couplGUU )
!                       Bm(dn_,i6)=Bm(dn_,i6) * ( propZ*couplZLL*couplZDD + propP*couplGLL*couplGDD )
!                   endif
!               endif
!           enddo
...
! spin & color avg., color convention adjustment
      mtrsq = mtrsq * alpha_s4Pi**2 /4d0/9d0
