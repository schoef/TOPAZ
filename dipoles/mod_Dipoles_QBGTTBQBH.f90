! this is the file to subtract dipoles for qbg->tt+qb+H amplitudes
      module ModDipoles_QBGTTBQBH
      use ModAmplitudes
      use ModProcess
      use ModParameters
      use ModMisc
      ...
      public :: EvalDipoles_QBGTTBQBH

      logical, parameter :: invert_alphaCut = .false.
      logical, parameter,private :: TTBH_SpeedUp = .false.

      contains

!  we have a list of dipoles that we need to go through
!  We label things as 0-> bar t(p1)+ H(2) + t(3) + q(p4)+q(p5)+g(p6)
...
!-------------------------------------------------, the other for 'dn' quarks
      ...
      subroutine EvalDipoles_QBGTTBQBH(p,yRnDk1,Wgt,npdf1,sum_dip)
...
!------------------------------- here gluon emits
       ...
       if( TOPDECAYS.eq.0 ) sum_dip=sum_dip* 0.5d0!   this removes the factor two from ISFac which is introduced in mod_Process for speed-up
...
!      dipole subroutines
       ...
       if( first_time ) then
             ...
             call InitProcess_TbTGGH(ExtParticles(1:5))
...
!-- now Lorentz transform
     ...
     q(:,j) = k1
       ...
       IF( HDECAYS.NE.0 ) THEN
          call EvalPhasespace_HDecay(q(1:4,2),yRnDk(9:10),MomDK(1:4,7:8),PSWgt3)
...
!-----------------     initial   initial       final  top      top
  ...
  call Kinematics_TTBARH(0,(/-q(1:4,4),-q(1:4,5),q(1:4,2),q(1:4,1),q(1:4,3), Momzero,MomDK(1:4,1:8)/), (/4,5,3,1,2,0,7,8,9,10,11,12,13,14/), Not_Passed_Cuts,NBin(1:NumHistograms),PObs)
...
!         N2Jump = 1
!         if (HDECAYS.ge.1) then ! Z decays
!             N2jump=2
!         endif
!         if( TTBH_SpeedUp ) then
!           Nmax(2) = -1
!         endif
...
!------- fill in histograms
       ...
       complex(dp) :: HH(-1:1,-1:1)
       type(Particle),save :: ExtParticles(1:5)
       type(TreeProcess),save :: TreeAmpsDip(1:1)
       integer :: c1, c2, j1
       ...
       complex(8)        :: ResAmpsDip_mt(-1:1,1:2)
       complex(dp) :: POL1(-1:1,4),propZ,propP
       real(dp) ::  pjetout(4,5), weight,couplZLL,couplGLL,couplZUU,couplGUU,couplZDD,couplGDD
       real(dp), parameter :: CF=4.0_dp/3.0_dp
       integer :: Njet
       logical, save :: first_time = .true.
       ...
       if( first_time ) then
             call InitTrees(4,0,1,TreeAmpsDip,NumBoson=1)
             call InitProcess_TbTQbQH(ExtParticles(1:5))
             TreeAmpsDip(1)%PartRef(1:5) = (/1,5,2,3,4/)
...
!        if( HDECAYS.le.10 ) then  ! decaying on-shell Z
!             MZ_Inv = m_Z
!        elseif( HDECAYS.gt.10 ) then  ! decaying off-shell Z
!             MZ_Inv = dsqrt(dabs((q(1:4,2)).dot.(q(1:4,2))))
!        endif
...
!         N2Jump = 1
!         if (HDECAYS.ge.1 .or. HDECAYS.eq.-2) then ! Z decays
!             N2jump=2
!         endif
!         if( TTBH_SpeedUp ) then
!           Nmax(2) = -1
...
!     if( HDECAYS.gt.0 ) call ZGamLCoupl(1,hel(2),couplZLL,couplGLL)  ! charged lept
...
!           propZ = (1d0,0d0)/(MZ_Inv**2-m_Z**2+ci*Ga_ZExp*m_Z)   * MZ_Inv**2
!           propP = (1d0,0d0)
!       endif
...
!               Bm_dn(i1,i6,i2,i3,i4,i5)=Bm(i1,i6,i2,i3,i4,i5)
! 
!               if( HDECAYS.gt.0 .and. HDECAYS.lt.10 ) then
!                   Bm_up(i1,i6,i2,i3,i4,i5)=Bm_up(i1,i6,i2,i3,i4,i5)*propZ*couplZLL*couplZUU
!                   Bm_dn(i1,i6,i2,i3,i4,i5)=Bm_dn(i1,i6,i2,i3,i4,i5)*propZ*couplZLL*couplZDD
!               elseif( HDECAYS.gt.10 ) then
!                   Bm_up(i1,i6,i2,i3,i4,i5)=Bm_up(i1,i6,i2,i3,i4,i5)* ( propZ*couplZLL*couplZUU + propP*couplGLL*couplGUU )
!                   Bm_dn(i1,i6,i2,i3,i4,i5)=Bm_dn(i1,i6,i2,i3,i4,i5)* ( propZ*couplZLL*couplZDD + propP*couplGLL*couplGDD )
!               endif
!           endif      
...
SUBROUTINE InitProcess_TbTGGH(ExtParticles)
use ModProcess
implicit none
type(Particle) :: ExtParticles(:)
  ...
  ExtParticles(5)%PartType = Hig_
  ExtParticles(5)%ExtRef   = 5
  ExtParticles(5)%Mass = m_H
  ExtParticles(5)%Mass2= ExtParticles(5)%Mass**2


RETURN
END SUBROUTINE InitProcess_TbTGGH
...
!      if (HDECAYS.ge.1) then
!           call HDecay(ExtParticles(5),DK_LO,MomDK(1:4,7:8))
!      endif
...
SUBROUTINE InitProcess_TbTQbQH(ExtParticles)
use ModProcess
implicit none
type(Particle) :: ExtParticles(:)
