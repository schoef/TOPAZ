...
C  
C FOR PROCESS : u u~ -> t t~ h  
C  
C Crossing   1 is u u~ -> t t~ h  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C             WRITE(*,*) IHEL,T
      ...
      REAL*8 FUNCTION UUB_TTBH(P,NHEL,IC)
C  
C FUNCTION GENERATED BY MADGRAPH
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      CALL FVIXXX(W(1,4   ),W(1,6   ),GG ,TMASS   ,TWIDTH  ,W(1,7   ))     
      CALL IOSXXX(W(1,7   ),W(1,3   ),W(1,5   ),GHTOP ,AMP(1   ))          
      CALL FVOXXX(W(1,3   ),W(1,6   ),GG ,TMASS   ,TWIDTH  ,W(1,8   ))     
      CALL IOSXXX(W(1,4   ),W(1,8   ),W(1,5   ),GHTOP ,AMP(2   ))          
      JAMP(   1) = -AMP(   1)-AMP(   2) 
      UUB_TTBH = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          ...
          ENDDO
          UUB_TTBH =UUB_TTBH+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
