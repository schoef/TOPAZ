...
!! RR added 05/02/2014 -- id tag for massive colorless particle
!! present tags: 1==vector/axial vector
!!               2==vector/axial vector incl q-dependent interactions (non-renorm)
!!               3==scalar/axial
integer, public, save :: ColorlessTag=2
...
!!!! BSM top-Z couplings
real(8), public :: DeltaF1A, DeltaF1V,DeltaF2V,DeltaF2A    !, DeltaF2A, DeltaF2V    ! later
...
ELSEIF( COLLIDER.EQ.5 ) THEN
  Collider_Energy  = 350d0*GeV

ELSEIF( COLLIDER.EQ.6 ) THEN
  Collider_Energy  = 500d0*GeV

ELSEIF( COLLIDER.EQ.7 ) THEN
  Collider_Energy  = 1000d0*GeV
...
integer PartType
   ...
   elseif( abs( PartType) .eq. SBot_) then
      GetMass = m_SBot
   elseif( abs( PartType) .eq. ElM_) then
      GetMass = 0d0
