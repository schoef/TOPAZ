! ./BinnedLogL GetLogL LO_Hi17_V+0.00_A+0.00.dat LO_Hi17_V+4.00_A+0.20.new.dat 17 3000 20000 15.0 test
...
! MARKUS test: implment max.LL instead of LL ratio test
  ...
  real(8) :: LLRatio_array(1:2,1:MaxExp),LLRatio_array_tmp(1:2,1:MaxExp),LLRatio_min,LLRatio_max,WhichBin,LL
...
!-------------------------------------------------------------------------
  ...
  do while(.not.eof(112))!   reading input file 1
     ...
     if( NHisto(1).ne.TotSigmaHisto ) cycle
     NumBins0=NumBins0 + 1
  enddo
...
!  call random_seed()
  do iHypothesis=1, 1
     ...
     do iPseudoExp = 1, ExpectedEvents(1,:)
...
!      LLRatio=0d0
     do iBin=BinMin,BinMax
        if (ObsEvents(iHypothesis,iBin) .ne. 0 .and. ExpectedEvents(1,iBin) .ne. 0 .and. ExpectedEvents(2,iBin) .ne. 0) then
              ...
              LLR = LL + ObsEvents(iHypothesis,iBin)*dlog(dble(ExpectedEvents(2,iBin))) - dble(ExpectedEvents(2,iBin))
