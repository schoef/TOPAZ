...
SUBROUTINE EvalSqAmp_qqb_ttb_pho()
use ModMisc
use ModProcess
implicit none
...
RETURN
END SUBROUTINE EvalSqAmp_qqb_ttb_pho
...
!                      TheTreeAmp%NumSca = TheTreeAmp%NumSca + 1   ! this is suppoed to be done outside this subroutine
                  ...
                  elseif( IsABoson(TheTreeAmp%PartType(NPart)) ) then! careful: bosons should only be places *between* same flavor quark lines
                     ...
                     if( abs(TheTreeAmp%PartType(NPart)).eq.abs(Hig_)) TheTreeAmp%NumV = TheTreeAmp%NumV + 1
...
! pause
   ...
   if( present(SecondPol) .and. present(SecondResult) ) then
        if( IsAQuark(TheTreeProcess%PartType(1)) ) then
            SecondResult = psp1_(Pol(1:4),SecondPol(1:4))
        elseif( TheTreeProcess%PartType(1).eq.Glu_ ) then
