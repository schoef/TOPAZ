...
SUBROUTINE HDecay(ZBoson,Topol,MomDK)
use ModParameters
use ModMisc
use ModProcess
implicit none
...
!    PropPh = (1d0,0d0)! the photon propagator is included as 1/kV^2 term in the decay matrix element
! 
...
!       couplZTT_left2_dyn  = couplZTT_left2  * PropZ *couplZFF
!       couplZTT_right2_dyn = couplZTT_right2 * PropZ *couplZFF
!       
!    elseif( ZDecays.gt.10 ) then  ! Z is off-shell
!       couplZTT_left_dyn  = couplZTT_left * PropZ *couplZFF + Q_Top*PropPh*Q_el
!       couplZTT_right_dyn = couplZTT_right*PropZ  *couplZFF + Q_Top*PropPh*Q_el
!       couplZTT_left2_dyn  = couplZTT_left2 *PropZ*couplZFF + couplGaTT_left2 *PropPh*Q_el
!       couplZTT_right2_dyn = couplZTT_right2*PropZ*couplZFF + couplGaTT_right2*PropPh*Q_el
!    endif
! 
! 
!     if( Topol.eq.DK_LO ) then 
!         if( abs(ZBoson%Helicity).ne.1 ) call Error("invalid Z boson polarization in ZDecay",ZBoson%Helicity)
!         ZBoson%Pol(1:4)  = ZGamPolVec(dcmplx(MomDK(1:4,1)),dcmplx(MomDK(1:4,2)),ZBoson%Helicity) ! &                                  ! -g_{mu,nu} term
! !                          - ZBoson%Mom(1:4) * ((ZBoson%Mom(1:4)).dot.(ZBoson%Pol(1:4)))/(ZBoson%Mom(1:4).dot.ZBoson%Mom(1:4))        ! +k_mu k_nu/(k.k) term (zero for massless fermions)
!         ZBoson%Pol(5:16) = 0d0
!     else
...
!     couplZUU_A=(couplZUU_left-couplZUU_right)/2d0     *1d0
!     couplZDD_V=(couplZDD_left+couplZDD_right)/2d0     *1d0
!     couplZDD_A=(couplZDD_left-couplZDD_right)/2d0     *1d0
! 
